        ...
        abt::State const* state, abt::HistoricalData const* data)
{
    ...
    if (data) {
        heuristicInfo->currentStep = static_cast<PositionData const*>(data)->getCurrentStep();
        heuristicInfo->action = static_cast<PositionData const*>(data)->getAction();
    }
...
std::unique_ptr<abt::HistoricalData> RobotModel::createRootHistoricalData()
{
    ...
    return std::make_unique<PositionData>(this,
                                          initialRobotState,
                                          nullptr,
                                          0,
                                          true);
