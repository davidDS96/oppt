...
public:
    ...
    void serialize(std::string& path) override {
        ...
        *outFilePtr << "numObstacles: " << numObstacles << endl;
...
struct RandomObstaclesStats: public Stats {
public:
    RandomObstaclesStats() {
        ...
        for (auto & entry : algMap) {
            ...
            if (entry.second.size() > 0) {
                ...
                for (auto & stat : entry.second) {
                    outFile << static_cast<RandomObstaclesStats*>(stat.get())->discountedReward << " \n";
                }
...
private:
    ...
    FloatType getMeanDiscountedReward_(const VectorStats& stats) {
        ...
        for (auto & stat : stats) {
            meanDiscountedReward += static_cast<RandomObstaclesStats*>(stat.get())->discountedReward;
        }
    ...
    unsigned int getNumberOfSuccessfulRuns_(const VectorStats& stats) {
        ...
        for (auto & stat : stats) {
            if (static_cast<RandomObstaclesStats*>(stat.get())->successfulRun) {
                numSuccessfulRuns++;
            }
        }
...
public:
    ...
    void readLogfiles() {
        ...
        parseRandomObstacleLogFiles(path_);
    ...
    void parseRandomObstacleLogFiles(std::string& path) {
		...
		cout << "process: " << logFile << endl;
                std::shared_ptr<Stats> stats = std::make_shared<RandomObstaclesStats>();
                ...
                if (logFileValid(pathToLogFile)) {
                    ...
                    static_cast<RandomObstaclesStats*>(stats.get())->discountedReward = findDiscountedReward(pathToLogFile);
                    static_cast<RandomObstaclesStats*>(stats.get())->successfulRun = findSuccessfulRun(pathToLogFile);
    ...
    std::shared_ptr<Stats> parseLogFile(std::string& path) {
        ...
        if (algorithmString.find("measureCompareObst") != std::string::npos) {
            ...
            unsigned int numObstacles = readNumObstacles(path);
            measureCompareStats->numObstacles = numObstacles;
...
private:
    ...
    unsigned int readNumObstacles(std::string& path) {
        std::ifstream input(path);
        unsigned int numObstacles = 0;
        for (std::string line; getline(input, line);) {
            if (line.find("numObstacles") != std::string::npos) {
                std::vector<std::string> elems;
                split(line, ' ', elems);
                oppt::printVector<std::string>(elems, "elems");
                numObstacles = (unsigned int)atof(elems[elems.size() - 1].c_str());
                return numObstacles;
            }
        }
