...
public:
	...
	controlDuration_ =
        ...
        discreteCollisionFunction_ = DubinCollisionFunction(std::bind(&Robot::makeDiscreteCollisionReportDirty,
                                     robotEnvironment_->getRobot()));
    ...
    virtual PropagationResultSharedPtr propagateState(const PropagationRequest* propagationRequest) const override {        
        ...
        if (propagationRequest->enableCollision) {
            bool collided = collides(denormalizedInputState, dnS);
            propagationResult->madeCollisionReport = true;
            propagationResult->collided = collided;
            if (collided && propagationRequest->allowCollisions) {
...
private:
    ...
    // The collision function to use
    DubinCollisionFunction discreteCollisionFunction_ = nullptr;
    ...
    bool collides(const RobotStateSharedPtr& state1, const RobotStateSharedPtr& state2) const {
        auto state1N = robotEnvironment_->getRobot()->getStateSpace()->normalizeState<VectorState>(state1);
        auto state2N = robotEnvironment_->getRobot()->getStateSpace()->normalizeState<VectorState>(state2);
        unsigned int n = 5;
        //return continuousCollisionFunction_(state1N, state2N, n)->collides;
        FloatType t = 0;
        for (unsigned int i = 0; i != n; ++i) {
            t += 1.0 / (FloatType)n;
            auto interpolatedStateN =
                robotEnvironment_->getRobot()->getStateSpace()->interpolate(state1N, state2N, t);
            auto interpolatedStateD =
                robotEnvironment_->getRobot()->getStateSpace()->denormalizeState<VectorState>(interpolatedStateN);
