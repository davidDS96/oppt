...
public:
    ...
    /**
     * @brief If this parameter is set to false, particles that collide with an obstacle are not considered
     */
...
public:
     ...
     */
    FilterResultPtr filter(const FilterRequestPtr& filterRequest) override;

    FilterResultPtr propagateParticles(const FilterRequestPtr& filterRequest);
};
