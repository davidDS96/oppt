        ...
        for (size_t i = particleCounter; i != deficit; ++i) {
            ...
            if (propagationRequest->allowCollisions || !collided || filterRequest->allowTerminalStates) {
                ...
                if (propagatedParticle->getWeight() > 0 || filterRequest->allowZeroWeightParticles) {
                    ...
                    RobotStateUserDataSharedPtr userData = replenishedParticlesVec[particleCounter]->getState()->getUserData();
                    if (userData) {
                        userData->previousState = sampledParticles[i]->getState();
                    } else {
...
FilterResultPtr ParticleFilter::filter(const FilterRequestPtr& filterRequest)
{
    ...
    if (particleCounter == 0) {
        ...
        WARNING("Couldn't sample any particles");
        return filterResult;
    }
    ...
    // Resampling step
    auto resampledParticles = replenishedParticles.sampleWeighted(filterRequest->randomEngine, filterResult->particles.size());
