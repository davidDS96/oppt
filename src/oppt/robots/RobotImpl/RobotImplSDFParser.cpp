    ...
    while (modelElement) {
        ...
        while (linkElement) {
            ...
            while (collisionElement) {
                std::string collisionName = linkName + "::" + collisionElement->Get<std::string>("name");
        ...
        const sdf::ElementPtr& visualElement) const
{
    ...
    Matrixdf pose = Matrixdf::Identity(4, 4);

    GeometrySharedPtr boxGeometry = std::make_shared<oppt::geometric::Box>(visualName, sizeVec, pose);
        ...
        const sdf::ElementPtr& visualElement) const
{
    ...
    if (radiusElement) {
        ...
        Matrixdf pose = Matrixdf::Identity(4, 4);
        GeometrySharedPtr sphereGeometry = std::make_shared<oppt::geometric::Sphere>(visualName, radius, pose);
        ...
        const sdf::ElementPtr& visualElement) const
{
	...
	Matrixdf pose = Matrixdf::Identity(4, 4);
        GeometrySharedPtr cylinderGeometry = std::make_shared<oppt::geometric::Cylinder>(visualName, radius, length, pose);
        ...
        const sdf::ElementPtr& visualElement) const
{
    ...
    GeometrySharedPtr meshGeometry = std::make_shared<oppt::geometric::Mesh>(visualName, meshUri, pose, scale);
