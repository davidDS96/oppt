...
public:
    ...
    /**
     * Parses an add obstacle change
     * @param ifstream Input stream to the changes file
     * @param sdfString SDF containting a description of the obstacle that is being added
     */
    bool parseAddObstacle(std::ifstream& ifstream, std::string& sdfString);

    /**
     * Parses an remove obstacle change
     * @param ifstream Input stream to the changes file
     * @param obstacleName The name of the obstacle that is being removed
     */
    bool parseRemoveObstacle(std::ifstream& ifstream, std::string& obstacleName);

    /**
     * Parses an change obstacle pose change
     * @param ifstream Input stream to the changes file
     * @param obstacleName The name of the obstacle whose pose is being changed
     * @param poseVec A 6D vector containing the new pose
     */
    bool parseChangePose(std::ifstream& ifstream, std::string& obstacleName, VectorFloat& poseVec);
