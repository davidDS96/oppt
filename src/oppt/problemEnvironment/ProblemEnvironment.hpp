...
public:
    ...
    RobotEnvironment const* getRobotPlanningEnvironment() const {
    ...
    void updateViewer(const RobotStateSharedPtr& currentState,
        ...
        VectorFloat particleColor = VectorFloat( {0.2, 0.6, 0.2, 0.1});
        ...
        for (size_t i = 0; i != subStates.size(); ++i) {
            ...
            for (size_t j = 0; j != numDrawnParticles; ++j) {
                ...
                if (opptBeliefState) {
                    ...
                    if (userData) {
                        auto previousParticleState = userData->previousState;
