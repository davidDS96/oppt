...
inline FloatType euclideanDistance(const FloatType* s1, const FloatType* s2, const size_t& dim)
{
    FloatType sum = 0;
    [](const FloatType * s1, const FloatType * s2, FloatType & sum, const size_t & dim) {
        FloatType diff = 0.0;
...
template<typename T>
inline Quaternionf eulerAnglesToQuaternion(T& angles)
{
    Matrixdf rotX(4, 4);
    Matrixdf rotY(4, 4);
    Matrixdf rotZ(4, 4);
