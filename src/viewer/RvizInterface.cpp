...
bool RvizInterface::setupViewer(const std::string& modelFile,
                                const std::string& environmentFile)
{
    /**if (!viewerSetup_) {
        if (!initializeRos()) {
            cout << "RvizInterface: Couldn't initialize ros" << endl;
            assert(false);
        }

        dQueue_ = std::make_shared<CommandQueue>();
        viewerThread_ = new std::thread(&RvizInterface::run, this, dQueue_);
        //publishEnvironmentThread_ =
        //    new std::thread(&RvizInterface::publishEnvironment, this);
        while (!viewerLoaded_) {
            cout << "wait for viewer" << endl;
            sleep(1);
        }

        //setupEnvironment(environment_file, model_file);
        //setupRobot(model_file);
    }
